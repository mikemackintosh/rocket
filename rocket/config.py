...
from rocket.utils import isstr, raw
...
class Config(collections.Mapping):
    ...
    @staticmethod
    def etc_path(*args):
        return Config.mount_path(*('etc', 'rocket') + args)

    @staticmethod
    def mount_path(*args):
        return os.path.join(*(os.environ.get('ROCKET_MOUNT_PATH', '/'),) + args)

    @staticmethod
    def user_path(*args):
        return os.path.join(*(os.path.expanduser('~/.rocket'),) + args)

    @staticmethod
    def runtime(*args, **kwargs):
        from rocket.runtime import Runtime
    ...
    def evaluate(data):
        ...
        for _ in range(5):
            ...
            if not changed:
                break
        if changed:
            raise ConfigError('.rocket.yml circular reference')

    def setup_path(self):
        if not self.data.get('path'):
            path = '.'
            while os.path.split(os.path.abspath(path))[1]:
                config_path = os.path.join(path, '.rocket.yml')
    ...
    def setup(self):
        ...
        data = {}
        # runtime
        yml_path = os.path.join(self.data['path'], '.rocket.yml')
        if yml_path and os.path.isfile(yml_path):
            data = self.parse(yml_path)
            if not isinstance(data, dict):
                raise ConfigError('.rocket.yml syntax error')
        ...
        # parse configs
        runtime_config = self.parse(runtime.path('rocket.yml'))
        rocket_config = self.parse(data_path, require_exists=False)
        ...
        self.data = {
            'env': {
                'ROCKET_ENV': self.env,
                'ROCKET_PATH': data['path'],
                'ROCKET_RUNTIME': data['runtime'],
            },
        }
        # merge runtime
        self.merge(runtime_config, self.data)
        # merge runtime config
        self.merge(rocket_config, self.data)
