...
import pipes
import re
from rocket.config import Config
from rocket.exceptions import ConfigError
from rocket.utils import Shell, isstr
...
class Project(object):
    ...
    def run(self, section, argv=None):
        ...
        with Shell() as shell:
            ...
            if section != 'run':
                # raw arguments
                shell.write('export ROCKET_ARGV=%s' %
                ...
                for arg in argv:
                    if arg.startswith('--'):
                        ...
                        args.append(arg)
                # parsed arguments
                shell.write('export ROCKET_ARGS=%s' %
                ...
                for i, arg in enumerate(args):
                    shell.write('export ROCKET_ARG%s=%s' % (i, pipes.quote(arg)))
                # parsed argument options
                for name, value in opts.items():
                    shell.write('export ROCKET_ARGS_%s=%s' %
                                (name, pipes.quote(value)))
                # parsed options
                shell.write('export ROCKET_OPTS=%s' %
                            pipes.quote(' '.join(opts_list)))
                shell.write('export ROCKET_CWD=%s' % pipes.quote(os.getcwd()))
            # execute script
            shell.write('# script')
            shell.write(script)
